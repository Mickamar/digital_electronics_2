...
int main() {
    ...
    while (1) {
        ...
        if (detector == 0) { //no light, diod shine
            uart_puts("LED svítí\n");
            GPIO_write_high(&PORTB, LAMP);
        } else {
            uart_puts("LED nesvítí\n");
        ...
        // Kontrola času pro zapnutí lampy od 18:00:00 do 06:59:59 nebo pokud war == 0
        if ((hours >= 18 && hours <= 23) || (hours >= 0 && hours < 7) || detector == 0) {
            uart_puts("Zapínání lampy mezi 18:00 a 7:00 nebo při war == 0\n");
            GPIO_write_high(&PORTB, LAMP); // Zapnutí lampy
        } else {
            uart_puts("LED nesvítí\n");
